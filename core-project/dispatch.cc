...
Dispatch::work()
{
  ...
  // Decode wasn't ready i guess?
  if (!in_ready)								
    return;
  
  // queue is empty. we have no work to do so return
  if (dd_uop_queue.empty())				
    return;
  ...
  while(i < dd_uop_queue.size() &&\
        dd_uop_queue.front().address == front_addr) {
    ...
    if (Op.opcode == UD_Icall || Op.opcode == UD_Iret || Op.opcode == UD_Ipop\
        || Op.opcode == UD_Ipush || Op.opcode == UD_Ipushfq) {
      Op.reg3 = UD_R_RSP;
      Op.value3 = regfile.rsp;
    }
