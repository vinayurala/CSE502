...
Execute::do_syscall()
{
  ...
  switch(prev_reg1){
  case UD_R_RAX:
    regfile.rax = Core::syscall(regfile.rsp, Op.address, prev_value1, 
                                regfile.rdi, regfile.rsi, regfile.rdx,
                                regfile.r10, regfile.r8, regfile.r9);
    break;
  case UD_R_RDX:
    regfile.rax = Core::syscall(regfile.rsp, Op.address, regfile.rax,
                                regfile.rdi, regfile.rsi, prev_value1, 
                                regfile.r10, regfile.r8, regfile.r9);
    break;
  case UD_R_RSP:
    regfile.rax = Core::syscall(prev_value1, Op.address, regfile.rax,
                                regfile.rdi, regfile.rsi, regfile.rdx,
                                regfile.r10, regfile.r8, regfile.r9);
    break;
  case UD_R_RSI:
    regfile.rax = Core::syscall(regfile.rsp, Op.address, regfile.rax, 
                                regfile.rdi, prev_value1, regfile.rdx,
                                regfile.r10, regfile.r8, regfile.r9);
    break;
  case UD_R_RDI:
    regfile.rax = Core::syscall(regfile.rsp, Op.address, regfile.rax, 
                                prev_value1, regfile.rsi, regfile.rdx,
                                regfile.r10, regfile.r8, regfile.r9);
    break;
  case UD_R_R8:
    regfile.rax = Core::syscall(regfile.rsp, Op.address, regfile.rax,
                                regfile.rdi, regfile.rsi, regfile.rdx,
                                regfile.r10, prev_value1, regfile.r9);
    break;
  case UD_R_R9:
    regfile.rax = Core::syscall(regfile.rsp, Op.address, regfile.rax,
                                regfile.rdi, regfile.rsi, regfile.rdx,
                                regfile.r10, regfile.r8, prev_value1);
    break;
  case UD_R_R10:
    regfile.rax = Core::syscall(regfile.rsp, Op.address, regfile.rax,
                                regfile.rdi, regfile.rsi, regfile.rdx,
                                prev_value1, regfile.r8, regfile.r9);
    break;
  default:
    regfile.rax = Core::syscall(regfile.rsp, Op.address, regfile.rax,
                                regfile.rdi, regfile.rsi, regfile.rdx,
...
Execute::work()
{
      ...
      case UD_Iret:
        {
          ...
          clear_out = true;
#ifdef DEBUG_LOAD
          cout<<"\nRet. Stack returned "<<hex <<returnaddr;
          cout<<" at stackpointer "<<Op.value3<<endl;
          dump_op(Op);
          dump_values();
#endif
          return;
...
#ifdef DEBUG_LOAD
          cout<<"\nPop. Stack returned "<<hex<<Op.value2;
          cout<<" at stackpointer "<<Op.value3<<endl;
          dump_op(Op);
          dump_values();
#endif
  ...
  Op = de_uop_queue.front();
   ...
   if(Op.address != expected_addr && !firstrun) {
     ...
     cout<<__FILE__<<"\nUnexpected address! Expected: "<<hex<<expected_addr;
     cout<<" Got: "<<Op.address<<endl;
#endif
     de_uop_queue.pop_front();
     return;
   }
   ...
   cout<<__FILE__<<"\n0 Expected: "<<hex <<expected_addr;
   cout<<" Got: "<<Op.address <<" Op.size: "<<Op.opsize<<endl;
   dump_op(Op);
   dump_values();
#endif
   ...
   case UD_Idiv:
     {
       // need to deal with both rax and rdx
       if(Op.value3 != 0) {
   ...
   case UD_Ipop:
     {
       ...
       state = STATE_READ;
#ifdef DEBUG
       cout<<__FILE__<<"POP instruction" <<hex <<"Addr "<<Op.address<<" size ";
       cout<<Op.opsize<<endl <<"Reg1 "<<Op.reg1 <<" value1 "<<Op.value1;
       cout<<" Reg2 "<<Op.reg2 <<" Value2 "<<Op.value2 <<" Immed "<<Op.immed;
       cout<<" Address " <<Op.address <<" Op.reg3 "<<Op.reg3 <<" Op.Value3 ";
       cout<<Op.value3<<endl;
#endif
       return;
     }
   ...
   case UD_Iload:
     {
       ...
       //stahp = true;
#ifdef DEBUG_TEMP
       cout<<__FILE__<<"Load instruction" <<hex <<"Addr "<<Op.address<<" size ";
       cout<<Op.opsize<<endl <<"Reg1 "<<Op.reg1 <<" value1 "<<Op.value1;
       cout<<" Reg2 "<<Op.reg2 <<" Value2 "<<Op.value2 <<" Immed "<<Op.immed;
       cout<<" Address " <<Op.address <<" Op.reg3 "<<Op.reg3 <<" Op.Value3 ";
       cout<<Op.value3<<endl;
   ...
   case UD_Icmp:	// Check CF SF ZF and OF.
     {
       ...
       //ERROR? IMPLEMENT 'OF' Flag.
       if ((unsigned long long)Op.value2 \
           < (unsigned long long)Op.value1) 
         eflags.set(MASK_CF);
       else
         eflags.reset(MASK_CF);
       ...
       if ((!sign_op1 && !sign_op2 && sum_sign)\
           || (sign_op1 && sign_op2 && !sum_sign))
         eflags.set(MASK_OF);
       else
         eflags.reset(MASK_OF);
       break;
   ...
   default:
     {
       cout<<__FILE__<<"Couldn't run this unknown instruction: ";
       cout<<Op.opcode<<" at address "<<hex<<Op.address<<dec<<endl;
   ...
   dump_op(Op);
   cout <<__FILE__ <<"1 New expected Address: "<<hex <<expected_addr;
   cout<<" Current Address: "<<Op.address<<endl;	
#endif
